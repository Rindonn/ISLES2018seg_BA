...
@ LastEditors: Rindon
@ LastEditTime: 2024-09-09 14:35:22
@ Description: 
'''
...
training_part, testing_part = torch.utils.data.random_split(dataset_m, (420,82), generator=torch.Generator().manual_seed(34)) # random_split(数据集，长度)随机将一个数据集分割成给定长度的不重叠的新数据集
testset = testing_part
testloader = DataLoader(testset)

model = TransUnet()
model.load_state_dict(torch.load(r'D:\ISLES2018seg_BA\record\Others\ba_TransUnet_300-fold-2.pth'))
