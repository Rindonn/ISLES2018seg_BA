...
@ LastEditors: Rindon
@ LastEditTime: 2024-04-18 15:10:13
@ Description: 
'''
import torch
...
class swinTUNet(nn.Module):
    def __init__(self):
        ...
        self.conv7 = nn.Conv2d(64, 128, 3, padding=1, bias=False)
        ...
        self.swint = swinT.SwinT(in_channels=128, input_resolution=(32,32), num_heads=4, 
                    window_size=4, qkv_bias=False, drop=0.1,
                    attn_drop=0.1, drop_path=0.1,downsample=False)
        #############
        
        #Bottleneck #
        self.convB1 = nn.Conv2d(128, 128, 3, padding=1, bias=False)
        self.normB1 = nn.BatchNorm2d(128)
        '''#relu
        self.convB2 = nn.Conv2d(128, 128, 3, padding=1, bias=False)
        self.normB2 = nn.BatchNorm2d(128)'''
        #relu
        self.convB3 = nn.Conv2d(128, 128, 3, padding=1, bias=False)
        self.normB3 = nn.BatchNorm2d(128)
        ...
        '''#relu
        self.conv17 = nn.Conv2d(32, 32, 3, padding=1, bias=False)
    ...
    def forward(self, x):
       ...
       # x = self.convB2(x)
        ...
        #x = self.conv17(x)
        #x = F.relu(self.norm17(x))
        x = self.conv18(x)
        x = F.relu(self.norm18(x))
