...
def train_model():
    ...
    for fold, (train_ids, validation_ids) in enumerate(kfold.split(training_part)):
        ...
        df.to_csv(f'ba_swinTXNet_300-fold-{fold}.csv',index=False)  
        ...
        optimizer = AdamW(model.parameters(), lr=0.0000423, weight_decay=0.05)
        ...
        for epoch in range(300):
            ...
            # Saving the model
            save_path = f'ba_swinTXNet_300-fold-{fold}.pth'
            ...
            with torch.no_grad():
                ...
                if max_acc < float('%.3f' % acc):
                    ...
                    print("save!")
                    # 保存模型语句
                    torch.save(model.state_dict(),f"ba_swinTXNet_best_{fold}.pth")
            ...
            file.to_csv(f'ba_swinTXNet_300-fold-{fold}.csv', mode='a', header=False, index=False)
        #记录模型最好的信息
