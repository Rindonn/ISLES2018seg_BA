...
def train_model():
    ...
    for fold, (train_ids, validation_ids) in enumerate(kfold.split(training_part)):
        ...
        df.to_csv(f'ba_modTAU_skip1_300-fold-{fold}.csv',index=False)  
        ...
        for epoch in range(300):
            ...
            # Saving the model
            save_path = f'ba_modTAU_skip1_300-fold-{fold}.pth'
            ...
            with torch.no_grad():
                ...
                if max_acc < float('%.3f' % acc):
                    ...
                    print("save!")
                    # 保存模型语句
                    torch.save(model.state_dict(),f"ba_modTAU_skip1_best_{fold}.pth")
            ...
            file.to_csv(f'ba_modTAU_skip1_300-fold-{fold}.csv', mode='a', header=False, index=False)
        #记录模型最好的信息
